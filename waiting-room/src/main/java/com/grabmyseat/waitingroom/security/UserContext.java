...
public record UserContext(Long userId, Set<String> roles) {
    ...
    public static UserContext fromRequest(HttpServletRequest request) {
        ...
        Long userId = userIdHeader == null || userIdHeader.isBlank() ? null : Long.valueOf(userIdHeader);
        String rolesHeader = request.getHeader(ROLES_HEADER);
        Set<String> roles = rolesHeader == null || rolesHeader.isBlank()
                ? Set.of()
                : Arrays.stream(rolesHeader.split(",")).map(String::trim).filter(r -> !r.isEmpty()).collect(Collectors.toSet());
